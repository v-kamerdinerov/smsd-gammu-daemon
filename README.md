...
Если вас не пугают слова — плейбук, ansible, инвентори — вы по адресу. После клонирования репозитория можно прогнать плейбук и получить вышеописанную инсталляцию, но уже посредством автоматики. Важно — заполнить `group_vars` — в них укажем так же токен от нашего бота и номер телефона по желанию.
```bash
cd playbooks/
