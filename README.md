...
## 📦 Содержание
- [📝 Преамбула](#-преамбула)
- [⚙️ У нас было...](#%EF%B8%8F-у-нас-было)
...
- [⚙️ Автоматизация с Ansible](#%EF%B8%8F-автоматизация-с-ansible)
- [💡 Полезные советы и рекомендации](#-полезные-советы-и-рекомендации)
  - [Настройка udev правил](#настройка-udev-правил)
- [⚠️ Ошибки и устранение неполадок](#%EF%B8%8F-ошибки-и-устранение-неполадок)
  - [Потеря связи с модемом](#потеря-связи-с-модемом)
  - [Модем не видно в системе](#модем-не-видно-в-системе)
...
## ⚙️ У нас было...
<p align="center">
  <img src="https://avatars.mds.yandex.net/get-vthumb/2455229/ec6a9a38a6eef596fa8105ce9c74b794/800x450" alt="Модем и Orange Pi">
</p>
...
2. Убедитесь, что модем определяется системой, используя команду:
   ...
   ```
   Прекрасно, модем определился — мы запомним из данного вывода `manufacturer id` данного устройства — `12d1:1001`. `12d1` — это код поставщика для Huawei, а строка, идущая за этим кодом (в моём случае — `1001`) — это ID продукта. Если система, после подключения модема, создаст `/dev/ttyUSB0`, это значит, что всё сделано правильно. Проверить это можем с помощью команд:
   ```bash
   dmesg | grep tty
...
1. Чаще всего одноплатники поставляются на Debian/Ubuntu подобных дистрибутивах — поэтому ставим пакеты для Gammu и используя пакетный менеджер `apt`.
   Хорошим тоном будет обновить репозитории и ОС:
   ```bash
...
1. Gammu обладает широким спектром возможностей по взаимодействию с модемами и телефонами. Нам же нужен тривиальный функционал обработки SMS сообщений. Конфигурационный файл лежит по пути `/etc/gammu-smsdrc`:
   ```ini
...
3. Ключевым тут является скрипт, который запускается при каждом полученном сообщении — `RunOnReceive = /opt/my-awesome-python-script.py`. Его пример можно найти [здесь](./playbooks/roles/gammu-smsd/templates/sms-to-telegram.py.j2). В нём необходимо заполнить лишь следующее — номер телефона и данные нашего TG-бота, который мы, конечно же, заблаговременно [создали заранее~нет~](https://www.directual.com/lesson-library/how-to-create-a-telegram-bot).
   ...
   ```
4. Осталось запустить `systemd` демон для работы Gammu:
   ```bash
   ...
   ```
5. Проверкой результатов будет отправка любого сообщения на ваш номер и трепетное ожидание его в вашем ТГ канале.
   <p align="center">
     <img src="./imgs/message.png" alt="Пример сообщения в Telegram">
   </p>

## ⚙️ Автоматизация с Ansible
Если вас не пугают слова — плейбук, ansible, инвентори — вы по адресу. После клонирования репозитория можно прогнать плейбук и получить вышеописанную инсталляцию, но уже посредством автоматики. 
Важно — заполнить `group_vars` — в них укажем так же токен от нашего бота и номер телефона по желанию.
Подробнее - [работа с ansible.](./playbooks/README.md)
...
У модемов есть свойство — менять свой `ttyID` в зависимости от фазы луны, солнцестояния и курса доллара. Хорошим решением будет зафиксировать этот самый `ttyID` с помощью правил `udev`. При подключении модема к одноплатнику мы выяснили `manufacturer id` нашего устройства.
```bash
Bus 008 Device 001: ID 1d6b:0003 Linux Foundation 3.0 root hub
Bus 007 Device 003: ID 12d1:1001 Huawei Technologies Co., Ltd. E161/E169/E620/E800 HSDPA Modem
Bus 007 Device 001: ID 1d6b:0002 Linux Foundation 2.0 root hub
...
```
Следующим правилом мы говорим, что устройство вендора Huawei и пределённым `id` стоит вешать на симлинк `/dev/sms`. Теперь выполним релоад данных правил:
```bash
udevadm control --reload-rules
udevadm trigger
```
По пути `/dev/sms` мы должны наблюдать симлинк на `tty` от модема:
```bash
ls -hal /dev/sms
...
### Потеря связи с модемом
Иногда происходит потеря связности модема — и нашего одноплатника. Порой это связано с потерей интернет соединения, порой с волшебными процессами внутри модема — отвалы наблюдаются как сообщения в логах `systemd` — `gammu-smsd`:
```bash
"Probably the phone is not connected. (TIMEOUT[14])"
"Error at init connection:"
"Error getting SMS status:"
```
При этом рестарт сервиса `systemd` нам не поможет — могла произойти потеря связи с самим модемом, и потребуется физически переподключить модем, что чаще всего невозможно. Но мы не отчаиваемся — ведь у нас HAA-система, SLA 99.9999%! Решением стал немного костыльный, но зато надёжный, как автомат Калашникова, сервис — `watcher`. Его код можно найти [здесь](./playbooks/roles/gammu-smsd/files/watcher.py). Данный сервис сканирует лог с момента запуска демона `gammu-smsd` на предмет указанных выше ошибок. Если он их находит, то выполняет программное "переподключение" модема через USB-разъём и перезапускает сервис `gammu-smsd` в `systemd`.
...
Не забываем добавить новые юниты в демон `systemd`:
...
Большинство USB-модемов при подключении к компьютеру, по умолчанию, работают в режиме устройства хранения данных (storage mode), а нам нужен режим модема (modem mode). Для того чтобы понять, в каком именно режиме работает ваш модем — подключите его к настольному компьютеру. Если система монтирует его в виде USB-диска (обычно содержащего установщики разных программ и драйверов) — вам надо переключить его в режим модема. Переключение режима устройства в различных модемах выполняется немного по-разному. Обычно это решается командой:
```bash
